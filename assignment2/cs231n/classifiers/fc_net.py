...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            ...
            self.params['W'+str(i+1)] = np.random.normal(0.0, weight_scale,(in_dim,out_dim))
            self.params['b'+str(i+1)] = np.zeros(out_dim)
        if self.normalization and 1< self.num_layers-1:
    ...
    def loss(self, X, y=None):
        ...
        for i in range(layers-1):
            ...
            X, caches['c'+str(i+1)] = generic_forward(X, W, b, \
            gamma, beta, bn_param, dropout_param)

        W = self.params['W'+str(layers)]
        b = self.params['b'+str(layers)]
        scores, caches['c'+str(layers)] = affine_forward(X,W,b)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dx, dw, db= affine_backward(dout,caches['c'+str(layers)])
        grads['W'+str(layers)] = dw
        grads['b'+str(layers)] = db

        for i in range(layers-1)[::-1]:
            dx, dw, db, dgamma, dbeta = generic_backward(dx, \
            caches['c'+str(i+1)])
            grads['W'+str(i+1)] = dw
            grads['b'+str(i+1)] = db
