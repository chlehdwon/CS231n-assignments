...
def svm_loss(x, y):
    ...
    correct_class_scores = x[np.arange(num_inputs), y].reshape(num_inputs,1)
    x = np.maximum(0,x-correct_class_scores+1)
    x[np.arange(num_inputs),y]=0
    loss = np.sum(x)
    loss /= num_inputs
    dx = np.where(x>0,1,0)
    dx[np.arange(num_inputs),y] -= np.sum(dx,axis=1)
    dx = dx/num_inputs
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
